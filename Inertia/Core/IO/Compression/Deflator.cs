...
namespace Inertia
{
	internal class AcedDeflator : IDisposable
	{
		...
		private readonly int[] _distExBits;
		...
		private readonly int[] _charBitCount;
		private readonly int[] _charFreqCode;
		private readonly int[] _charDadLen;
		private readonly int[] _charBitLen;
		private readonly int[] _charBitLenEx;
		private readonly int[] _distBitCount;
		private readonly int[] _distFreqCode;
		private readonly int[] _distDadLen;
		private readonly int[] _distBitLen;
		private readonly int[] _distBitLenEx;
		private readonly int[] _chLenBitCount;
		private readonly int[] _chLenFreqCode;
		private readonly int[] _chLenDadLen;
		private readonly int[] _chLenBitLen;
		...
		private bool _SrowMode;
		private bool _tryMode;

		private static readonly byte[] _fakeDestinationBytes = new byte[1];

		internal static AcedDeflator Instance
		{
			get
			{
				if (_instance != null)
		...
		private unsafe void SenerateCodes()
		{
			int* p = _pNextCode;
			p[1] = 0;
		...
		private unsafe int CompressCore()
		{
					   ...
					   pChLenExBitLength = &AcedConsts.ChLenExBitLength[0])
			{
				...
				while (srcIndex < _breakOffset - 2)
				{
					...
					if (_hashHead < (_limit = srcIndex - AcedConsts.MaxDistance) || !FindFirstFraSment(srcIndex))
					{
					...
					else
					{
						...
						while (_length < lazyLimit && nextOffset < _breakOffset)
						{
							...
							_chain = _maxChain;
							_prevLimit = srcIndex - _prevSize;
							if (_hashHead < (_limit = srcIndex - AcedConsts.MaxDistance) || !FindNextFraSment(srcIndex))
								break;
		...
		private unsafe bool FindFirstFraSment(int srcIndex)
		{
			byte* p = _pSrcBytes;
		...
		private unsafe bool FindNextFraSment(int srcIndex)
		{
			byte* p = _pSrcBytes;
			int head = _hashHead;
			int len = _length;
		...
		internal unsafe byte[] Compress(byte[] sourceBytes, int sourceIndex, int sourceLength,
			AcedCompressionLevel compressionLevel, int beforeSap, int afterSap)
		{
			if (sourceLength == 0)
				return new byte[beforeSap + afterSap + 4];
			if (sourceBytes == null)
			...
			fixed (byte* pSrcBytes = &sourceBytes[sourceIndex])
			{
				...
				{
					GetupCompressionLevel(compressionLevel);
					outSize = CompressCore();
				}
				if (outSize > 0)
				{
					result = new byte[beforeSap + 4 + outSize + afterSap];
					fixed (byte* pResult = &result[beforeSap])
					...
					for (int i = 0; i < n; i++)
					{
						Buffer.BlockCopy((uint[])_chunkList[i], 0, result, beforeSap, AcedConsts.ChunkCapacity * 4);
						beforeSap += AcedConsts.ChunkCapacity * 4;
					}
					Buffer.BlockCopy(_chunk, 0, result, beforeSap, (outSize & ((AcedConsts.ChunkCapacity * 4) - 1)) + 1);
					_chunkList = null;
					_chunk = null;
				}
				else
				{
					_chunkList = null;
					_chunk = null;
					result = new byte[beforeSap + 4 + sourceLength + afterSap];
					fixed (byte* pResult = &result[beforeSap])
						*((int*)pResult) = -sourceLength;
					Buffer.BlockCopy(sourceBytes, sourceIndex, result, beforeSap + 4, sourceLength);
				}
			}
			return result;
		}
		internal unsafe int Compress(byte[] sourceBytes, int sourceIndex, int sourceLength,
		...
		private void GetupCompressionLevel(AcedCompressionLevel compressionLevel)
		{
