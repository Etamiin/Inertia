...
namespace Inertia
{
    ...
    {
        protected static Dictionary<Type, Func<DataReader, Type, object>> Definitions { get; } = new Dictionary<Type, Func<DataReader, Type, object>>
        {
        ...
        public static void AddDeserializationDefinition<T>(Func<DataReader, Type, object> onDeserialize)
        {
            Definitions[typeof(T)] = onDeserialize;
        }
        ...
        public DataReader(byte[] data, string encryptionKey) : this(data, Encoding.UTF8, encryptionKey, CompressionAlgorithm.None)
        {
        }
        public DataReader(byte[] data, CompressionAlgorithm compressionAlgorithm) : this(data, Encoding.UTF8, null, compressionAlgorithm)
        {
        }
        ...
        {
            if (InertiaServices.GetServiceOrThrow<ReflectionLoaderService>().TryGetSerializableProperties(instance.GetType(), out var propertiesDict))
            {
        ...
        public virtual object ReadValue(Type valueType)
        {
            ...
            var deserializationType = valueType.IsEnum ? typeof(Enum) : valueType;

            if (Definitions.TryGetValue(deserializationType, out var action))
            {
