...
namespace Inertia
{
    public class DataWriter : IDisposable
    {
        ...
        public static void AddSerializationDefinition<T>(Action<DataWriter, object> onSerialize)
        {
        ...
        public virtual DataWriter Write(object value, Type type)
        {
            ...
            var serializationType = type.IsEnum ? typeof(Enum) : type;

            if (Definitions.TryGetValue(serializationType, out var action))
            {
                action(this, value);    
                return this;
            }
            ...
            throw new NotSupportedException($"Type '{type.Name}' not supported, you can build a custom serializer for this type by using '{nameof(AddSerializationDefinition)}' method.");
        }
        ...
        public virtual DataWriter WriteAutoSerializable(object value)
        {
            ...
            if (InertiaServices.GetServiceOrThrow<ReflectionLoaderService>().TryGetSerializableProperties(value.GetType(), out var propertiesDict))
            {
