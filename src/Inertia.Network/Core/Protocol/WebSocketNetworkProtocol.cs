...
namespace Inertia.Network
{
    public abstract class WebSocketNetworkProtocol : NetworkProtocol
    {
        ...
        private protected WebSocketNetworkProtocol(ILogger logger)
        {
        ...
        public override bool TryParseMessage(NetworkEntity receiver, DataReader reader, MessageParsingOutput output)
        {
                ...
                while (reader.UnreadedLength > 0)
                {
                    ...
                    using (var messageReader = new DataReader(applicationData))
                    {
                        while (messageReader.UnreadedLength > 0)
                        {
                            ...
                            message.Deserialize(messageReader.ReadByte(), messageReader);
            ...
            {
                Logger.Error(ex, GetType(), nameof(TryParseMessage));
        ...
        public byte[] WriteWsMessage(byte[] applicationData, WebSocketOpCode wsOpCode)
        {
            using (var writer = new DataWriter())
            {
                ...
                opCode.SetBitByRef(0, true, EndiannessType.BigEndian);
